...
export async function initDatabase() {
  ...
  if (Number(sessionCount.cnt) === 0) {
    ...
    await queryRun(sessionInsertSql, [adminId]);
  }
...
export async function getActiveSessionByAdmin(adminId) {
  return queryGet("SELECT * FROM sessions WHERE admin_id = ? AND status != 'archived' ORDER BY id DESC LIMIT 1", [adminId]);
}

export async function getSessionById(id) {
  return queryGet('SELECT * FROM sessions WHERE id = ?', [id]);
}

export async function getAdminById(id) {
  return queryGet('SELECT * FROM admins WHERE id = ?', [id]);
}
...
export async function updateAdminPassword(adminId, password) {
  await queryRun('UPDATE admins SET password = ? WHERE id = ?', [password, adminId]);
  return getAdminById(adminId);
}

export async function updateAdminName(adminId, name) {
  await queryRun('UPDATE admins SET name = ? WHERE id = ?', [name, adminId]);
  return getAdminById(adminId);
}

export async function setAdminToken(adminId, token) {
  await queryRun('UPDATE admins SET token = ? WHERE id = ?', [token, adminId]);
}
...
export async function deleteAdmin(adminId) {
  ...
  try {
    const sessions = await getSessionsByAdmin(adminId);
    for (const session of sessions) {
      await queryRun('DELETE FROM active_connections WHERE session_id = ?', [session.id]);
      await queryRun('DELETE FROM responses WHERE session_id = ?', [session.id]);
      await queryRun('DELETE FROM sessions WHERE id = ?', [session.id]);
    }
    await queryRun('DELETE FROM admins WHERE id = ?', [adminId]);
...
export async function getSessionsByAdmin(adminId) {
  return queryAll('SELECT * FROM sessions WHERE admin_id = ? ORDER BY id DESC', [adminId]);
}
...
export async function createSessionForAdmin(adminId, name) {
  ...
  await queryRun(closeSql, [adminId]);
  ...
  const row = await queryGet(insertSql, [adminId, name || `교육 ${new Date().toLocaleDateString('ko-KR')}`]);
...
export async function getResponseByDevice(sessionId, deviceId) {
  return queryGet('SELECT * FROM responses WHERE session_id = ? AND device_id = ?', [sessionId, deviceId]);
}

export async function getResponseById(id) {
  return queryGet('SELECT * FROM responses WHERE id = ?', [id]);
}

export async function deleteResponse(id) {
  const response = await getResponseById(id);
  if (!response) return null;
  await queryRun('DELETE FROM responses WHERE id = ?', [id]);
  return response;
}
...
export async function deleteTodayResponsesBySession(sessionId) {
  ...
  const result = await queryRun(sql, [sessionId]);
  await queryRun('DELETE FROM active_connections WHERE session_id = ?', [sessionId]);
...
export async function createResponse(sessionId, deviceId) {
  const row = await queryGet(
    ...
    [sessionId, deviceId, 'in_progress', '{}']
  );
...
export async function getResponsesBySession(sessionId) {
  return queryAll('SELECT * FROM responses WHERE session_id = ? ORDER BY created_at DESC', [sessionId]);
}
...
export async function getTodayResponsesBySession(sessionId) {
  ...
  return queryAll(sql, [sessionId]);
}

export async function getCompletedResponses(sessionId) {
  return queryAll("SELECT * FROM responses WHERE session_id = ? AND status = 'completed' ORDER BY completed_at DESC", [sessionId]);
}
...
export async function upsertConnection(id, sessionId, page) {
  const sql = isPostgres
    ...
    : `INSERT INTO active_connections (id, session_id, page, last_seen)
       ...
       ON CONFLICT(id) DO UPDATE SET session_id = excluded.session_id, page = excluded.page, last_seen = excluded.last_seen`;
  await queryRun(sql, [id, sessionId, page]);
}
...
export async function getActiveConnectionCount(sessionId) {
  ...
  const row = await queryGet("SELECT COUNT(*) as cnt FROM active_connections WHERE session_id = ? AND page NOT LIKE '/admin%' AND page NOT LIKE '/share%'", [sessionId]);
  return row?.cnt || 0;
}
...
export async function getDashboardStats(sessionId) {
  ...
  const total = await queryGet(`SELECT COUNT(*) as cnt FROM responses WHERE session_id = ? ${todayClause}`, [sessionId]);
  const completed = await queryGet(`SELECT COUNT(*) as cnt FROM responses WHERE session_id = ? AND status = 'completed' ${todayClause}`, [sessionId]);
  const inProgress = await queryGet(`SELECT COUNT(*) as cnt FROM responses WHERE session_id = ? AND status = 'in_progress' ${todayClause}`, [sessionId]);
  const online = await getActiveConnectionCount(sessionId);
